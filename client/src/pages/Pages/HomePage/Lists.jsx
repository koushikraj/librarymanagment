...
const Lists = ({ user }) => {
  ...
  const addToCart = async () => {
    ...
    console.log(send);
    await axios
      .post(`${baseApiURL()}/addToCart`, send, {})
      .then((response) => {
  ...
  const fetchData = async () => {
    ...
    const response = await axios.get(`${baseApiURL()}/search/${search}`); //http://localhost:5000
    if (response.length == 0) {
      response = await axios.get(`${baseApiURL()}/allBook`); //http://localhost:5000/search/${search}
    }
